...
def app_factory(config: t.Optional[t.Dict[str, t.Any]] = None) -> Flask:
    ...
    After bootstrap, it's a good idea to never import from `run` or the source of the bootstrapped
    Flask application. Instead, all bootstrapped extensions should be accessed with Flask's `current_app`.
